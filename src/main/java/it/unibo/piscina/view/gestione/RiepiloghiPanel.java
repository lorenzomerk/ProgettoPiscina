...
public final class RiepiloghiPanel extends JPanel {
    ...
    private void createReports() {
        final List<String> names = controller.nomiRiepiloghi();
        for (int index = 0; index < names.size(); index++) {
            addReport(names.get(index), index);
        }
    }
    ...
    private void addReport(
        ...
        reports.add(new ReportTab(reportIndex, model));
    ...
    private void reloadSelected() {
        ...
        new SwingWorker<DatiTabella, Void>() {
            ...
            protected DatiTabella doInBackground() {
                return controller.eseguiRiepilogo(
                    report.index(), start, end
                );
            }
